...
const path1 = puzzleInput.split(/\n/)[0].split(',');

const path2 = puzzleInput.split(/\n/)[1].split(',');


let coordinates1 = [[0, 0, 0]];
let coordinates2 = [[0, 0, 0]];
let temp1 = [0, 0];
let temp2 = [0, 0];
let intersections = [];
let stepCounter1 = 0;
...
// Function to determine if two segments intesect, returns true or false
...
// Function that determines next coordinate
const pathfinder = (wire, coordinate, instruction) => {
  switch (instruction.slice(0, 1)) {
    case "U":
      coordinate[1] += +instruction.slice(1);
      break;
    case "D":
      coordinate[1] -= +instruction.slice(1);
      break;
    case "R":
      coordinate[0] += +instruction.slice(1);
      break;
    case "L":
      coordinate[0] -= +instruction.slice(1);
      break;
  ...
  wire.push([...coordinate, +instruction.slice(1)])
}

path1.map((instruction) => pathfinder(coordinates1, temp1, instruction));
path2.map((instruction) => pathfinder(coordinates2, temp2, instruction));


// Nested loop to check for intersections
for (let j = 1; j < coordinates1.length; j++) {
  stepCounter1 += Math.abs(coordinates1[j][2]);
  for (let k = 1; k < coordinates2.length; k++) {
    stepCounter2 += Math.abs(coordinates2[k][2]);
    
    if (intersects(coordinates1[j-1][0], coordinates1[j-1][1], coordinates1[j][0], coordinates1[j][1], coordinates2[k-1][0], coordinates2[k-1][1], coordinates2[k][0], coordinates2[k][1])) {

      // Checks which wire is horizontal then push intersection coordinates
      coordinates1[j-1][0] == coordinates1[j][0] ? intersections.push([coordinates1[j][0], coordinates2[k][1]]) : intersections.push([coordinates2[k][0], coordinates1[j][1]])
  
      // Calculates number of steps
      steps.push([(stepCounter1 - (coordinates1[j-1][0] == coordinates1[j][0] ? Math.abs(coordinates1[j][1] - coordinates2[k][1]) : Math.abs(coordinates1[j][0] - coordinates2[k][0]))), (stepCounter2 - (coordinates1[j-1][0] == coordinates1[j][0] ? Math.abs(coordinates2[k][0] - coordinates1[j][0]) : Math.abs(coordinates2[k][1] - coordinates1[j][1])))]);
    }
  }
...
// Part 1
console.log("What is the Manhattan distance from the central port to the closest intersection? Answer:", intersections.map(intersection => Math.abs(+intersection[0]) + Math.abs(+intersection[1])).sort((a,b) => a - b)[0]);

// // Part 2
console.log("What is the fewest combined steps the wires must take to reach an intersection? Answer:", steps.map(step => step[0] + step[1]).sort((a, b) => a - b)[0])
